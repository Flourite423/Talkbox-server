...
**接口**: `GET /api/get_messages?username=用户名&limit=50&before_id=消息ID`

**功能**: 获取用户相关的消息（私聊），支持无限滚动加载

**请求参数**: 通过 URL 查询参数传递
- `username`: 用户名（必填）
- `limit`: 每次加载的消息数量（可选，默认50，最大200）
- `before_id`: 获取此消息ID之前的消息（可选，用于加载更多）
...
**说明**:
- `has_more` 字段表示是否还有更多消息可加载
- 首次加载不传 `before_id`，获取最新消息
- 加载更多时传入当前最早消息的 `message_id`
...
**接口**: `GET /api/get_posts?page=1&page_size=20`

**功能**: 获取帖子列表，支持分页

**请求参数**: 通过 URL 查询参数传递（均可选）
- `page`: 页码（默认1）
